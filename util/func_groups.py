...
def print_fmt(*args):
    fmt = '{%-36s %-38s %5s %10s %-8s %6s %5s %3s 0, 0, 0, %1s %1s},'
...
def print_null():
    fmt = '{%s, "%s", "%s", "%s", "%s", "%s", "%s", 0, 0, 0, 0, %d, %d},'
    print fmt[:-1] % ("NULL", "NULL", "NULL", "NULL", "NULL", "NULL", "NULL", 0, 0)
    ...
    for stats in ["", "_stats"]:
        for alg in ["nw", "sg", "sw"]:
            ...
            for isa in ["sse2", "sse41", "avx2"]:
                ...
                print_fmt(pre,         pre,         alg+stats, "orig", "NA", "32", "32", 1, is_stats, 1)
                print_fmt(pre+"_scan", pre+"_scan", alg+stats, "scan", "NA", "32", "32", 1, is_stats, 0)
                bits = isa_to_bits[isa]
                for par in ["scan", "striped", "diag"]:
                    widths = [64, 32, 16, 8]
                    for width in widths:
                        name = "%s_%s_%s_%s_%s" % (pre, par, isa, bits, width)
                        print_fmt(name, name, alg+stats, par, isa, bits, width, bits/width, is_stats, 0)
                print_null()
                print "};"
...
1, is_stats, 0)
            ...
            for par in ["scan", "striped", "diag"]:
                ...
                print_fmt(name, name, alg+stats, par, "sat", "NA", 8, -1, is_stats, 0)
            print_null()
            print "};"
            print 'static parasail_function_group_t %s_%s = {"%s_%s", %s_%s_functions};' % ((pre, isa)*3)
